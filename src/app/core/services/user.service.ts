...
export class UserService {
  ...
  public async GetAll(): Promise<void> {
    try {
      const result = await lastValueFrom(this._httpClient.get<User[]>(`${environment.apiUrl}user`));
  ...
  public async Update(id: number, user: Partial<User>): Promise<void> {
    try {
      await lastValueFrom(this._httpClient.put(`${environment.apiUrl}user/${id}`, user));
  ...
  public async Remove(id: number): Promise<void> {
    try {
      await lastValueFrom(this._httpClient.delete(`${environment.apiUrl}user/${id}`));
